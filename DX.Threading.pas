...
type
  ...
  public
    constructor Create(ACommand: TCommand; ACompletionHandler: TCompletionHandler = nil;
      AErrorHandler: TErrorHandler = nil; ASynchronized: Boolean = true); reintroduce; overload;
    constructor Create(ACommand: TCommand; const AObjetcsToFree: TDynObjectArray;
      ACompletionHandler: TCompletionHandler = nil; AErrorHandler: TErrorHandler = nil; ASynchronized: Boolean = true);
    ...
    /// main thread that is.
    /// </summary>
    property SynchronizeCompletionHandler: Boolean read FSynchronizeCompletionHandler
      write FSynchronizeCompletionHandler;
  end;
  ...
  public
    ...
    procedure Append(ACommand: TProc; const AObjectsToFree: TDynObjectArray;
      ACompletionHandler: TCompletionHandler = nil; AErrorHandler: TErrorHandler = nil;
      ASynchronized: Boolean = true); overload;
...
var
  GApplicationClass: TPersistentClass;
  GQueue: TAsyncCommandQueue;

procedure DoRunLoop;
begin
  if assigned(WakeMainThread) then
    WakeMainThread(nil);
  // if there is an Application class, then we are not in headless mode
  if assigned(GApplicationClass) then
  begin
    GApplicationClass.ClassName;
  end;
end;
...
procedure TAsyncCommandQueue.Append(ACommand: TProc; ACompletionHandler: TCompletionHandler = nil;
...
procedure TAsyncCommandQueue.Append(ACommand: TProc; const AObjectsToFree: TDynObjectArray;
  ACompletionHandler: TCompletionHandler = nil; AErrorHandler: TErrorHandler = nil; ASynchronized: Boolean = true);
begin
...
constructor TAsyncCommand.Create(ACommand: TCommand; const AObjetcsToFree: TDynObjectArray;
  ACompletionHandler: TCompletionHandler = nil; AErrorHandler: TErrorHandler = nil; ASynchronized: Boolean = true);
begin
  FCommand := ACommand;
...
constructor TAsyncCommand.Create(ACommand: TCommand; ACompletionHandler: TCompletionHandler;
  AErrorHandler: TErrorHandler; ASynchronized: Boolean);
var
  LObjects: TDynObjectArray;
begin
...
GApplicationClass := System.Classes.GetClass('TApplication');
FindGlobalComponent('Application');
